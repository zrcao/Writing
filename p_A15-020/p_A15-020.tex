...
The proposed scheme will use the time slot assignment schemes of existing TMACN tactical waveforms. SRW and WNW are all based on TDMA. The benefits of leveraging existing time slots assignment schemes have two folders. 
\begin{itemize}[noitemsep,nolistsep]
\item It simplifies IA technology insertion and assures the system is backward compatible. When 
\item It is a convenient method to designate the cluster head for an ad hoc IA clustering, which is the critical first step in the proposed SOIA scheme which has a simplified clustering strategy with low coordination overhead.
\end{itemize}
