...
TMACN are interference-limited due to the fact that multiple closely located radio assets need to share the transmission resources. Traditionally, time and frequency resources in wireless networks are divided into orthogonal units and assigned to different users, such as TDMA, FDMA and OFDMA. Each resource unit can only be used by a single transmission in order to avoid interference among multiple simultaneous transmissions. For large networks, resource reuse is widely adopted in both commercial and military systems to increase the network capacity. The same frequency and/or time resource unit is shared by multiple transceiver pairs that are far apart geographically such that mutual interference is not significant due to path loss. 

In addition to the frequency and the time signaling dimensions, multiple antenna arrays and MIMO systems enable the vector space as a third signaling dimension, i.e. the spatial dimension. The spatial dimension can be shared among multiple transmissions or multiple data streams of the same transceiver pair. Therefore, the same time and frequency resource unit can be shared among multiple transmissions as long as they can be separated in the spatial dimension. In the ideal case, each transmission occupies a subspace that is orthogonal to the subspaces of all other parallel transmissions. In reality, due to the correlations among antennas and limited channel diversity, the channel matrix may be rank-deficient thus limits the number of simultaneous transmissions that can be supported. To some extent, spread spectrum technology can also be viewed as using vector-based signaling to support multiuser communications. 

All aforementioned approaches are built upon the basic doctrine that, in order to avoid interference, closely located transceiver pairs in a wireless network must use orthogonal resource units for simultaneous transmissions. Each resource unit can only be used by a single transmission. Therefore, the number of transmissions at the same time are upper bounded by the number of orthogonal resource units. Given the limited bandwidth each radio can access, and the limited number of antennas each radio is equipped with due to the size, weight and power (SWaP) requirements, the capacity of tactical mobile networks is thus constrained during operational deployment. 
...
\subsection{Interference Alignment -- The Opportunity for Higher TMACN Throughput}

Interference alignment (IA)~\cite{4567589} is an emerging approach, which breaks away from the above fundamental doctrine, i.e., the number of simultaneous transmissions among closely located radio transceivers is limited by the number of orthogonal resource units.  

In interference alignment, the signaling of multiple closely located transceiver pairs are jointly designed, such that at each individual receiver all interference are aligned into a subspace that is orthogonal to the desired signal. The number of interferences is more than the rank of the aligned subspace. Using interference alignment, from the network point of view, the number of simultaneous transmissions within the interference range can be more than the number of available orthogonal resource units. Thus, the overall network throughput is increased. 

IA usually finds its uses in $K$-user interference channels and X-channels. Theoretic investigation has proven that IA can significantly improve the network capacity. For a $K$-user interference channels with varying channel gains, the IA scheme in~\cite{4567443} can achieve the total degree of freedom, which is $K/2$.

\subsection{Practical Challenges and Requirements}

The theoretic framework for IA has several idealistic assumptions that need to be addressed before apply IA in practical systems. The channel state information (CSI) is central to calculating IA precoders and equalizers. IA schemes usually require joint design of the transmission signals for multiple transmitters, as a result, IA schemes incur significant overhead for CSI information sharing. In a fast-fading mobile environment, the overhead of CSI acquisition can limit or cancel out the gains of IA. In addition, several practical challenges have to be addressed before materializing the throughput gain promised by IA in an operational TMACN.

{\textbf{\textit{Clustering transceiver pairs}}} --- IA requires coordination among multiple transceiver pairs to design the signals and setup the joint transmissions in the physical layer. So the first step in applying IA is to cluster transceiver pairs instead of individual nodes. This is different from the clustering problem in the traditional mobile ad hoc network (MANET). Even given the same group of radios, if the pairing of transceivers changes, the IA signaling of this group has to be re-designed. Most research papers discussing IA assume that the $K$ pairs are given {\it{a prior}}. This assumption is reasonable for a cellular system, where a centralized control plane manages multiple base stations and also has the full knowledge of all traffic. The knowledge of current traffic needs is necessary to determine the participating transceiver pairs within a cluster. TMACN, on the contrary, doesn't have a centralized control plane. Each radio in TMACN knows only its own traffic needs. There is no entity in a TMACN which has global knowledge of all traffic needs. In short, distributed pairing of radios with active traffic need is the first challenge to be addressed.
...
Task 1 is the leading tasking during the Phase I. It is expected that Task 1 will lead to a set of procedures each TMACN node operates during deployment to form IA clusters dynamically for simultaneous transmissions. In the following, the SOIA technical approach we proposed for Phase I effort are discussed. The approach includes three main components. First, identifying the leading transceiver pair of an IA cluster to be formed. Second, aligning interference at the head pair receiver and designing precoder for the transmitter. Third, selecting cluster member pairs and computing equalizers.
...
\item The training symbol matrix $\mathbf{X}_l$ is an $S_j$ by $S_j$ square matrix. For IA operations, $S_j<n_j$, where $n_j$ is the number of antennas on node $j$. In practical TMACN operations, the number antennas on each radio is limited. Thus, a limited set of training symbol matrices can be pre-installed onto each radio. Node $l$ broadcasts $S_j$ to all its neighbors such that node $k$ is able to select the correct training symbol matrix to estimate $\hat{\mathbf{P}}^{[k]}_{j,l}$. Task 2 defines details on the protocol frame structure in which $S_j$ is included in the header information.
...
Cascaded selection is the second approach that we plan to investigate during the phase I effort for cluster member selection. Similar to the centralized selection, in cascaded selection the receiver node of the cluster head pair $r$ first determines the set $\tilde{\mathcal{U}}$ according to Eq.~(\ref{tilde_U}). Different from the centralized approach, rather than selecting all pairs in $\tilde{\mathcal{U}}$ as members of the cluster, node $r$ chooses only one pair of nodes from $\tilde{\mathcal{U}}$ with the least interference energy projection. In other words, node $r$ selects pairs $u$ as the second member of the IA cluster according to
$$
...
During Phase I, we plan to implement the SOIA technology in Matlab and develop an Matlab-based simulator to evaluate the SOIA system performance. We recognize that it is extremely important to have a propagation model with high-fidelity in order to evaluate the system performance of the SOIA technology. The issue of a good multiuser propagation model has long been the challenge for many networking simulation tools, such as Qualnet and OpNet. The PI has extensive experience in realistic MIMO channel modeling using Ericsson's RUNE (Rudimentary Network Emulator)~\cite{ZandRRM:book:2001}, a MATLAB-based TDMA wireless networking simulation tool available to public. We will incorporate many features of RUNE into the SOIA simulator.
...
The 3GPP SCM was developed for cellular network simulation. However, we can adapt it to support system level simulation for mobile ad hoc networks. This adaptation has two steps. First, we need to lower the antenna height of the base stations nodes. In a TMACN edge networks, the antenna heights typically are between human height or vehicle tops. Second, instead fixing the roles among all radio nodes, i.e., base stations and mobile stations, all radio nodes in the SOIA simulator are peers.

For each node in an $N$-node simulation, the SOIA simulator will generate $N-1$ sets of MIMO channel coefficients according to procedures defined in~\cite{3GPP:SCM}, representing the channels between the node and the other $N-1$ nodes. In addition, the SOIA simulator will generate a $N$ by $N$ path gain matrix between all possible transceiver pairs. The path gain between any pair of radios is the sum of the distance-based path loss and the lognormal shadowing. For the distance based path loss, similar to the 3GPP Spatial Channel Model~\cite{3GPP:SCM}, we will use the COST 231 Walfish-Ikegami NLOS and LOS models. The lognormal shadowing for the NLOS has a standard deviation of 10dB, and that for LOS is 4dB, as given in~\cite{3GPP:SCM}. Furthermore, for the same transmitter, two closely located receivers have correlated lognormal shadowing. We use a pre-generated correlated lognormal map, from which each tactical receiver's lognormal shadowing can be efficiently derived based on its location. %Fig.~\ref{fig:lognormalmap} shows an example of correlated lognormal shadowing map over an area of 300 meters by 300 meters.

\subsubsection{Mobility Trace and Vignette}

Due to the mobility, the channels and the path losses between any pair of nodes changes. The SOIA simulator uses a complex value (in other words, a 2-dimensional real vector) to represent the speed and the direction of a node. The L2 norm of the complex value represents the absolute value of the speed (m/s); and the direction of the complex value represents the mobility direction. For each slot, changes in the speed and its direction of each radio are generated based on the average speed and acceleration of each radio. As a result, each radio will have a new position in the 2-D plan for every TDMA slot. The new positions are then used in the physical channel model to calculate the received signal strength at each tactical radio from all potential transmitters. The pace of channel variations can thus be controlled by the speed of the mobility. For low speed mobility, new positions and old positions overlap with each other such that the channel keeps constant for longer period.

The mobility traces not only can be randomly generated from the Matlab simulator, but also can be incorporated from real operation collections as well as Google Earth KML files. This feature enable us to evaluate the SOIA system performance in a realistic tactical maneuver. We plan to build a simulation vignette based on a cordon and search mission. In this tactical scenario, insurgent forces have tried to establish improvised explosive device (IED) cells and factories. The 1st Platoon of the C Company is conducting a cordon and search mission in the area to disrupt and defeat insurgent forces and provide stability and security. Three squads are position close by so they form a tactical network. Two squads are cordoning the road while the third squad searching to high value targets. The network throughput with and without SOIA technology will be compared against the mobility traces and traffic patterns distilled from this cordon and search mission.

\subsubsection{Protocol Frame Structure}

An important component of Task 2 is to define the SOIA protocol frame structure. The frame should include head part dedicated for clustering and coordination among cluster members. Reducing the percentage of the frame header essentially motivated us to proposed the SOIA technology as explained in Task 1. 

Being able to reuse existing TMACN TDMA structures is the one of the key contributions we would like to delivery in the SOIA technology. To the purpose, we propose an innovative concept here for IA protocol frame structure. We define the SOIA frame logically instead of physically. As shown in Fig.~\ref{fig:SOIA frame}, the SOIA protocol frame has a fixed header, a variable-length header, and the rest is for IA data transmissions. The variable length header provides flexibility to accommodate different number of transceiver pairs in clusters.     

\begin{figure}[h]
\begin{center}
\includegraphics[width=.75\textwidth]{./figures/SOIAframe.png}
\caption{\small Illustrations of SOIA logic protocol frame structure and mapping the logic frame to physical TMACN TDMA slots.}
\label{fig:SOIA frame}
\end{center}
\end{figure}

Fig.~\ref{fig:SOIA frame} also depicts how we plan to map SOIA logic frames into the physical TMACN TDMA slots structures. Notably, the mapping is not continuous. A SOIA logic frame is segmented into sections and allocated with TDMA slots close in time but not necessary connected. The segments of one SOIA logic frames must be closely located in time thus the channel conditions are constant for each logic frame. On the other hand, allowing discontinuous mapping gives the TMACN TDMA resource allocation the flexibility to maintain its autonomous and fairness. We will work with our transition partner to investigate the mapping of SOIA logic frames onto tactical TDMA slot structures, e.g. soldier radio waveform.
...
\begin{figure}[h]
\begin{center}
\includegraphics[width=0.66\textwidth]{./figures/schedule.png}
\caption{\small Simuliation, modeling and implementation using the Matlab, System Generator design flow.}
\label{fig:schedule}
\end{center}
\end{figure}
...
\bibliography{ZCaoBibCollection.bib}

\includepdf[pages={1}]{GDLetter.pdf}
